/**
 * Single source of truth for customer-facing notification copy across
 * web admin (click-to-WhatsApp), RN admin (click-to-WhatsApp), and the
 * future Phase 8 v2 Resend automatic emails. All templates are short,
 * plain-text, and assume WhatsApp-compatible escaping (no markdown).
 */
...
export interface TemplateContext {
  ...
  bookingDate: string;
  /** Magic-link URL pointing at /booking-status?token=... */
  statusUrl?: string;
  /** Free-text amount string, already formatted (e.g. "₹50,000") */
  amount?: string;
  /** Free-text reason supplied by admin or system */
  reason?: string;
  /** For conflict notifications: the winning booking's identifier */
  winnerBookingId?: string;
  /** Latest comment message text (for comment notifications) */
  commentText?: string;
}

export interface RenderedTemplate {
  /** Short subject (used by email; ignored by WhatsApp) */
  subject: string;
  /** Plain-text body */
  body: string;
}
